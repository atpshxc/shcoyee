	...
	private $error = array();
  	...
  	private function getList() {
		...
		$data = array(
			...
			'filter_sku'	  => $filter_sku,
			'filter_price'	  => $filter_price,
			'filter_tax_money'	  => $filter_tax_money,
			'filter_quantity' => $filter_quantity,
			'filter_status'   => $filter_status,
			'filter_category_id' =>$filter_category_id,
			'sort'            => $sort,
		...
		foreach ($results as $result) {
			...
			$this->data['products'][$result['product_id']] = array(
				...
				'columnsid' => $result['columnsid'],//zhangbo edit 增加栏目
				'name'       => $result['name'],
				'model'      => $result['model'],
				'sku'      => $result['sku'],
				'price'      => $result['price'],
		...
		$this->data['filter_tax_money'] = $filter_tax_money;
  	...
  	private function getForm() {
    	...
    	$this->product_setter(isset($product_info)?$product_info : NULL, 'classid',0);//zhangbo edit 增加分类
    	$this->product_setter(isset($product_info)?$product_info : NULL, 'columnsid',0);//zhangbo edit 增加栏目
		...
		$this->product_setter(isset($product_info)?$product_info : NULL, 'tax_money');
  	...
  	private function validateForm() {
    	...
    	if ((utf8_strlen(utf8_decode($this->request->post['model'])) < $rules['str_lenth']) ) {
