...
class ControllerAccountOrder extends Controller {
	...
	public function index() 
	{
		...
		if (isset($this->request->get['updateid']))
		{
			$this->model_account_order->updateOrder($this->request->get['updateid']);
		}
	...
	public function info() { 
		...
		$this->session->data['shuohuo_order_id']=$order_id;
		$this->load->model('account/order');
			...
			$this->data['payment_address'] = str_replace(array("\r\n", "\r", "\n"), '<br />', preg_replace(array("/\s\s+/", "/\r\r+/", "/\n\n+/"), '<br />', trim(str_replace($find, $replace, $format))));
      		...
      		if($order_info['payment_code']!=''&&!in_array($order_info['payment_code'],$this->direct_payments)&&$order_info['order_status_id']==$this->config->get('config_order_nopay_status_id'))
                ...
                if($order_info['order_status_id']=="3")
                        $this->data['shouhuo'] =$this->getChild('account/shouhuo');
      		...
      		foreach ($products as $product) {
        		...
        		$this->data['products'][] = array(
        			...
        			'href'    	 	   => $this->url->link('product/product', 'product_id=' . $product['product_id']),
          			...
          			'tax_money'         => number_format($product['tax'],2,'.',''),
