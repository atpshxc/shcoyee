...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
		...
		//if(true){
		if(!$product_data){
			$sql="SELECT DISTINCT *, pd.name AS name, p.image, p.tax_money, m.name AS manufacturer, (SELECT price FROM " . DB_PREFIX . "product_discount pd2 WHERE pd2.product_id = p.product_id AND pd2.customer_group_id = '" . (int)$customer_group_id . "' AND pd2.quantity = '1' AND ((pd2.date_start = '0000-00-00' OR pd2.date_start < NOW()) AND (pd2.date_end = '0000-00-00' OR pd2.date_end > NOW())) ORDER BY pd2.priority ASC, pd2.price ASC LIMIT 1) AS discount, (SELECT price FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$customer_group_id . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS special, (SELECT points FROM " . DB_PREFIX . "product_reward pr WHERE pr.product_id = p.product_id AND customer_group_id = '" . (int)$customer_group_id . "') AS reward, (SELECT ss.name FROM " . DB_PREFIX . "stock_status ss WHERE ss.stock_status_id = p.stock_status_id AND ss.language_id = '" . (int)$this->config->get('config_language_id') . "') AS stock_status, (SELECT wcd.unit FROM " . DB_PREFIX . "weight_class_description wcd WHERE p.weight_class_id = wcd.weight_class_id AND wcd.language_id = '" . (int)$this->config->get('config_language_id') . "') AS weight_class, (SELECT lcd.unit FROM " . DB_PREFIX . "length_class_description lcd WHERE p.length_class_id = lcd.length_class_id AND lcd.language_id = '" . (int)$this->config->get('config_language_id') . "') AS length_class, (SELECT AVG(rating) AS total FROM " . DB_PREFIX . "review r1 WHERE r1.product_id = p.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating, (SELECT COUNT(*) AS total FROM " . DB_PREFIX . "review r2 WHERE r2.product_id = p.product_id AND r2.status = '1' GROUP BY r2.product_id) AS reviews FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) LEFT JOIN " . DB_PREFIX . "manufacturer m ON (p.manufacturer_id = m.manufacturer_id) WHERE p.product_id = '" . (int)$product_id . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'";
			...
			if ($query->num_rows) {
				$product_data= array(
					'classid'       => $query->row['classid'],//zhangbo ecid 增加分类
	...
	public function getProducts($data = array()) {
		...
		if (!$product_data) {//zhangbo edit 增加分类
			//if (true) {
			...
			//zhangbo edit 添加分类
			if(isset($data['classs']))
			{
				if($data['classs']!=0)
				{
					$sql .= " AND p.classid ='".$data['classs']."'";
				}
			}
			
			//zhangbo edit 添加栏目 2015-07-27
			if(isset($data['columnsid']))
			{
				if($data['columnsid']!=0)
				{
					$sql .= " AND p.columnsid ='".$data['columnsid']."'";
				}
			}
			//zhangbo edit 添加品牌 2015-07-27
			if(isset($data['manufacturerid']))
			{
				if($data['manufacturerid']!=0)
				{
					$sql .= " AND p.manufacturer_id ='".$data['manufacturerid']."'";
				}
			}
...
//add 7/23 for 取得活动商品出售量
	public function getProductsTotalSeller($product_id) {
		$query = $this->db->query("SELECT op.product_id, COUNT(*) AS total FROM " . DB_PREFIX . "order_product op LEFT JOIN " . DB_PREFIX . "order o ON (op.order_id = o.order_id) LEFT JOIN " . DB_PREFIX . "product p ON (op.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE op.product_id = '" . (int)$product_id . "' AND o.order_status_id > '0' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' GROUP BY op.product_id");
...
//add 7/23 for 取得特价商品内容
	public function getDiscountProducts($columnsid, $limit) {
		$product_data = array();
		...
		$query = $this->db->query("select a.*, b.price as special, b.date_end, c.name, c.meta_title, c.meta_description from " . DB_PREFIX . "product a left join " . DB_PREFIX . "product_special b on (a.product_id = b.product_id ) left join " . DB_PREFIX . "product_description c on (a.product_id = c.product_id ) where b.customer_group_id = '" . (int)$customer_group_id . "' AND a.columnsid = '" . $columnsid . "' AND ((b.date_start = '0000-00-00' OR b.date_start < NOW()) AND (b.date_end = '0000-00-00' OR b.date_end > NOW())) ORDER BY b.priority ASC, b.price ASC LIMIT " . (int)$limit);
	...
	private function pinyin($zh){
		...
		if ($data) {
			$sql = "SELECT * FROM " . DB_PREFIX . "manufacturer";
                        ...
                        if (isset($data['columnsid']))
                        {
                            $sql.=" where columnsid=".$data['columnsid'] ;
                        }
