...
class ModelCheckoutOrder extends Model {
	...
	public function Gettotals($order_id)
        {
                        $order_query = $this->db->query("select * FROM " . DB_PREFIX . "order_total where order_id='".$order_id."'");
                        
                        $shipping=0;$sub_total=0;$tax=0;$total=0;
			foreach($order_query->rows as $ot)
			{
				if($ot['code']=='shipping')
					$shipping=$ot['value'];
        ...
        {
            $products = array();
            $order_query = $this->db->query("select op.*,sp.sku FROM ".DB_PREFIX."order_product op,s_product sp WHERE op.product_id=sp.product_id and order_id = '" .$order_id . "'");
            foreach($order_query->rows as $ot)
            {
                $products[] = array(
                    "price"=>$ot["price"],
	...
	public function getOrder($order_id) {
		...
		if ($order_query->num_rows) {
			...
			return array(
				...
				'payment_sn'                => $order_query->row['payment_sn']
			);
		} else {
			return false;	
		}
	...
	public function confirm($order_id, $order_status_id,$payment_sn='', $comment = '') {
...
//		$this->log->debug('IlexDebug:: Checkout order confirm() : order_id '.$order_id.' | order_status '.$order_status_id);
		...
		if ($order_info) {
			...
			$this->db->query("UPDATE `" . DB_PREFIX . "order` SET payment_sn='".$payment_sn."', invoice_no = '" . (int)$invoice_no . "', invoice_prefix = '" . $this->db->escape($order_info['invoice_prefix']) . "', order_status_id = '" . (int)$order_status_id . "', date_modified = NOW() WHERE order_id = '" . $order_id . "'");
