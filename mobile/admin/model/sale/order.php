...
class ModelSaleOrder extends Model {
	...
	public function editOrder($order_id, $data) {
        ...
        //张博
        public function updateOrder($order_id,$ghnb,$ghje) {
            //echo "UPDATE " . DB_PREFIX . "order SET isgouhui=1 where kjtorderid = '".$order_id . "'";exit;
				$this->db->query("UPDATE " . DB_PREFIX . "order SET isgouhui=1 where kjtorderid = '".$order_id . "'");
                                $this->db->query("delete from " . DB_PREFIX . "gouhui  where gouhuiorder = '".$order_id . "'");
                                $this->db->query("insert into " . DB_PREFIX . "gouhui set gouhuinumber='".$ghnb."',gouhuijine='".$ghje."',gouhuiorder='".$order_id."',gouhuidate='".date('y-m-d h:i:s',time())."'");  
	}
	...
	public function getOrder($order_id) {
		...
		if ($order_query->num_rows) {
			...
			return array(
				...
				'class_id'                => $order_query->row['class_id'],
...
public function Gettotals($order_id)
	...
	public function getOrders($data = array()) {
		$sql = "SELECT o.order_id,o.kjtorderid, o.customer_id, CONCAT(o.firstname) AS customer,o.email, o.shipping_firstname AS username ,(SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS status, o.total, o.currency_code, o.currency_value, o.date_added, o.date_modified FROM `" . DB_PREFIX . "order` o";
		...
		if (isset($data['filter_total']) && !is_null($data['filter_total'])) {
			$sql .= " AND o.total = '" . (float)$data['filter_total'] . "'";
                 ...
                 if (isset($data['filter_isgouhui']) && !is_null($data['filter_isgouhui'])) {
			$sql .= " AND isgouhui = '" . (float)$data['filter_isgouhui'] . "'";
		}
	...
	public function addOrderHistory($order_id, $data) {
		...
		if(isset($data['express'])&&$data['express']!=''&&(int)$data['express']!=0){
