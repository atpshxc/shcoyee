...
class ControllerSaleOrder extends Controller {
	...
	public function info() {
		...
		if ($order_info) {
			...
			$this->data['class_id'] =  $order_info['class_id'];
	...
	public function history() {
    	$this->load_language('sale/order');
		 unset( $this->data['noadd']);
		$this->load->model('sale/order');
	
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->user->hasPermission('modify', 'sale/order')) 
                { 
			//zhangbo
                        $statusid=(int)$this->request->post['order_status_id'];
                        if($statusid==2)//提交kjt通关
                        {
                            $order_info = $this->model_sale_order->getOrder($this->request->get['order_id']);
                            if($order_info["payment_sn"]=='')
                                {
                                    $this->data['noadd']='提交出关失败,没有支付流水号。';
                                }else
                                {
                                    unset( $this->data['noadd']);
                                    $this->creareorder($order_info);
                      ...
                      if(isset($this->data['noadd']))
                      {
                        $this->data['success'] = $this->data['noadd'];
                      }else
                      {
