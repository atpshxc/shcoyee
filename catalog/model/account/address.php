...
class ModelAccountAddress extends Model {
	public function addAddress($data,$customer_id=0) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "address SET customer_id = '" . $customer_id . "',  firstname = '" . $this->db->escape($data['firstname']) . "',address_1 = '" . $this->db->escape($data['address_1']) .  "', postcode = '" . $this->db->escape($data['postcode']) . "', city_id = '" . (int)$data['city_id'] . "', zone_id = '" . (int)$data['zone_id'] . "'," .
				"mobile = '" .  $this->db->escape($data['mobile']) ."', country_id = '" . (int)$this->config->get('config_country_id') . "'");
	...
	public function getAddress($address_id) {
		...
		if ($address_query->num_rows) {
			...
			if ($city_query->num_rows) {
				...
				$city_code = $city_query->row['city_code'];
			} else {
				$city = '';
				$city_code = '';
			}
