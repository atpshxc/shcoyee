...
class ModelAccountOrder extends Model {
	...
	public function getOrderProducts($order_id) {
		...
		$query = $this->db->query("SELECT o.*,p.tax_money,p.product_id,p.sku FROM " . DB_PREFIX . "order_product o left join " . DB_PREFIX . "product p on(o.product_id=p.product_id) WHERE o.order_id = '" . $order_id . "'");/* zhangbo edit*/
	...
	public function updateOrder($order_id,$orderstatus=0)
	{
            if($orderstatus==0)
		$this->db->query("UPDATE `" . DB_PREFIX . "order` SET order_status_id=7 WHERE order_id = '" . $order_id . "'");
            else
		$this->db->query("UPDATE `" . DB_PREFIX . "order` SET order_status_id=".$orderstatus." WHERE order_id = '" . $order_id . "'");
